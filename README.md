...
A function can be called by simply passing its arguments separated by a space (` `)
```
<< 1 2
```
...
Multiple lists can be intercalated by another list; that is multiple lists are appended using another list as a seperator
